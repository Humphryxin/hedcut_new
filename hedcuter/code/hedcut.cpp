...
/*void Hedcut::precompute_stipple(cv::Mat & img, int n, std::vector<cv::Point2d> & pts)
{
...
void Hedcut::create_disks(cv::Mat & img, CVT & cvt)
{
	...
	for (auto& cell : cvt.getCells())
	{
		...
		disk.radius = (uniform_disk_size) ? disk_size : (100.0f * disk_size / (avg_v + 100));
		//remember
