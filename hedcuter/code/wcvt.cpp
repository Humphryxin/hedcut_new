	...
	while (open.empty() == false)
	{
		...
		for (int dx =-1; dx <= 1; dx++) //x is row
		{
			...
			for (int dy = -1; dy <= 1; dy++) //y is column
			{
                ...
                cv::Point pt_tmp(x, y);
                // dist.at<float>(cpos.x, cpos.y)  	:  	cell dist: already initialized
                // color2dist(resizedImg, pt_tmp)  	: 	true value
                // oldd 							:	when uninitialzed positive infinite
				//float newd = dist.at<float>(cpos.x, cpos.y) + color2dist(resizedImg, pt_tmp);
				float newd = dist.at<float>(cpos.x, cpos.y) + decay_s*color2dist(resizedImg, pt_tmp);
				float oldd = dist.at<float>(x, y);

				if (newd < oldd)
				{
					//std::cout << "1: " << dist.at<float>(cpos.x, cpos.y) << "\t2: " << color2dist(resizedImg, pt_tmp) << "\toldd: " << oldd << std::endl;
...
void CVT::compute_weighted_cvt(cv::Mat &  img, std::vector<cv::Point2d> & sites)
{
	...
	if(iteration >= this->iteration_limit){
		std::cout << "Run out of iteration limit !!" << std::endl;
	}
	//if (debug) cv::waitKey();
}
